...
'dadc' : 'dadd #0, {reg}',
'sbc'  : 'subc #0, {reg}',
'jnz'  : 'jne {offset}' #ADDED: jne sometimes appears as jnz in objdump file
}
...
def asmMain(assembly, outfile=None, silent=False):
	...
	for ins in instructions.splitlines():
		...
		lineNumber += 1
	# print(f"output : {[hex(z) for z in output]}")
	#Resolve jump labels
	'''
...
def assembleEmulatedInstruction(ins):
	...
	elif '{offset}' in emulatedOpcodes[opcode]:
		offset = ins.split(" ")[1]
		ins = emulatedOpcodes[opcode]
		ins = ins.replace('{offset}', offset)
	else:
		ins = emulatedOpcodes[opcode]
	return assemble(ins)
...
def assembleOneOpInstruction(ins):
	...
	bits = int(''.join(str(e) for e in out), 2)
	appendWord(bits)
...
def assembleTwoOpInstruction(ins):
	...
	if '#' in regSrc:
		# need to convert to two's compliment if negative
		if '-' in regSrc:
			#ex: regSrc = #-52
			dec = int(regSrc[2:])
			twos_comp = hex((0xffff ^ dec)+1)[2:]
			regSrc = regSrc[0] + twos_comp
		else:
			regSrc = regSrc[0] + hex(int(regSrc[1:]))[2:]
	# print(f'regSrc: {regSrc}')

	extensionWordSrc, adrmodeSrc, regIDSrc = assembleRegister(regSrc)
	# print(f"\t extensionWordSrc: {extensionWordSrc}    adrmodeSrc: {adrmodeSrc}    regIDSrc: {regIDSrc}   ")
	...
	# print(f'regDest: {regDest}')

	extensionWordDest, adrmodeDest, regIDDest = assembleRegister(regDest, isDestReg = True)

	# print(f"\t extensionWordDest: {extensionWordDest}    adrmodeDest: {adrmodeDest}    regIDDest: {regIDDest}   ")
	...
	start = len(output)-1
	if extensionWordSrc:
		# print(f"extensionWordSrc: {extensionWordSrc}")
		appendWord(int(extensionWordSrc, 16))
		# print(f"extensionWordSrc: {extensionWordSrc}")
	if extensionWordDest:
		# print(f"extensionWordDest: {extensionWordDest}")
		appendWord(int(extensionWordDest, 16))
		# print(f"extensionWordDest: {extensionWordDest}")
	end = len(output)
	# print(f"({ins}) bits = {hex(bits)}")
	# print(f"out = {out}")
...
def assembleJumpInstruction(ins):
	...
	# MODIFIED -- file has $ before decimal offests 
	# print(f"ins: {ins}\tdest:{dest}")
	if '$' in dest:
		dest = dest.replace('$', '')
	...
	if char1 == '+' or char1 == '-' or char1 in [i for i in range(10)]:
		# offset = int(dest, 16)
		offset = int(dest) # MODIFIED -- objdump file has decimal offsets
		if offset % 2 != 0:
...
def getRegister(registerName):
	...
	if registerName.lower() in specialRegisterNames:
		...
		# print(f"registerName[1:]: {registerName[1:]}")
...
def assembleRegister(reg, opcode=None, isDestReg = False):
	...
	if '(' in reg: #Indexed mode (mode 1)
		...
		if isDestReg:
			...
			# MODIFIED: simplified since the @ is always the first character...
			# prev code was incorrect when reg was double
			regID = getRegister(reg[1:])
